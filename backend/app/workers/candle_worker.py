...
class SingleCandleWorker:
    ...
    def _bar_timestamp(self, bar: object) -> datetime | str | int | float | None:
    ...
    def sync_stock_incremental(
        ...
        *,
        ...
        sync_time = self._coerce_datetime(now)
        if not self._is_stock_incremental_window_open(
            timeframe=timeframe,
            at=sync_time,
        ):
            logger.info(
        ...
        end_at = self._latest_released_close(
            asset_class="stock",
            timeframe=timeframe,
            at=sync_time,
        )
        if end_at is None or self._is_before_first_stock_close(
            timeframe=timeframe,
            close_at=end_at,
        ):
            logger.info(
                "stock_incremental_skipped_awaiting_candle_close",
                extra={
                    "symbols": symbols,
                    "timeframe": timeframe,
                    "at": sync_time.isoformat(),
            ...
            return CandleSyncSummary(
                ...
                requested_symbols=tuple(symbols),
                upserted_bars=0,
                skipped_reason="awaiting_candle_close",
            )

        if not self._has_incremental_gap(
            asset_class="stock",
            symbols=symbols,
            timeframe=timeframe,
            close_at=end_at,
            ...
            logger.info(
                "stock_incremental_skipped_no_new_closed_candle",
                extra={
                    "symbols": symbols,
                    "timeframe": timeframe,
                    "at": sync_time.isoformat(),
                    "close_at": end_at.isoformat(),
                },
            )
            return CandleSyncSummary(
                asset_class="stock",
                ...
                requested_symbols=tuple(symbols),
                upserted_bars=0,
                skipped_reason="awaiting_next_close",
            )

        start = min(
            self._stock_incremental_start(
                symbol=symbol,
                timeframe=timeframe,
                now=sync_time,
            )
    ...
    def sync_crypto_backfill(
        ...
        *,
        ...
        total = 0
        for symbol in symbols:
            symbol_bars = adapter.fetch_ohlcv(
                symbol=symbol,
                timeframe=timeframe,
    ...
    def sync_crypto_incremental(
        ...
        *,
        ...
        for symbol in symbols:
            ...
            since_at = (
                self._coerce_datetime(state.last_candle_at) - interval
                if state is not None and state.last_candle_at is not None
                else end_at - (interval * self.settings.crypto_default_backfill_bars)
            )
            symbol_bars = adapter.fetch_ohlcv(
                symbol=symbol,
                timeframe=timeframe,
                since=int(since_at.timestamp()),
            )
            symbol_bars = self._filter_closed_bars(
                symbol_bars,
                timeframe,
    ...
    def _resolve_backfill_window(
        ...
        *,
        ...
        start_at = (
            end_at - (timeframe_to_timedelta(timeframe) * default_bars)
            if start is None
            else self._coerce_datetime(start)
        )
        return SyncWindow(start=start_at, end=end_at)

    def _stock_incremental_start(
        self,
        *,
        symbol: str,
        timeframe: str,
    ...
    def _is_stock_incremental_window_open(
        self,
        *,
        timeframe: str,
        at: datetime,
        ...
        return time(9, 30) <= local_time <= time(16, 5, 0)

    def _is_before_first_stock_close(
        self,
        *,
        timeframe: str,
        close_at: datetime,
        ...
        return local_close < local_close.replace(
            hour=9,
            minute=30,
            second=0,
            microsecond=0,
    ...
    def _latest_released_close(
        ...
        *,
        ...
        boundary = self._floor_close_boundary(
            release_reference,
            timeframe,
        ) - interval
