...
def evaluate(inputs: StrategyEvaluationInput):
    ...
    threshold = normalized_threshold(0.60, current_entry_policy(inputs))
    ...
    if momentum_20 is None or momentum_20 <= 0.01:
        blocked_reasons.append("momentum_too_weak")
    if trend_slope_20 is None or trend_slope_20 <= 0.004:
        blocked_reasons.append("trend_slope_too_weak")

    pullback_distance = percent_distance(close, ema_20)
    if pullback_distance is None or pullback_distance > 0.025:
        blocked_reasons.append("pullback_too_extended")
    signal_score = ratio_score(0.03 - min(pullback_distance or 0.03, 0.03), target=0.03)
    ...
    return build_outcome(
        ...
        payload={
            "pullback_distance_to_ema20": round(pullback_distance, 6) if pullback_distance is not None else None,
        },
    )
