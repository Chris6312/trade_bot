...
def evaluate(inputs: StrategyEvaluationInput):
    ...
    recent_high = highest_high(inputs.candles, lookback=5, exclude_latest=True)

    trend_score, participation_score, liquidity_score, stability_score = default_component_scores(inputs)
    threshold = normalized_threshold(0.64, current_entry_policy(inputs))
    ...
    if relative_volume_20 is None or relative_volume_20 < 1.1:
        blocked_reasons.append("participation_too_low")
    if momentum_20 is None or momentum_20 <= 0.015:
    ...
    signal_score = ratio_score(breakout_pct, target=0.015)
